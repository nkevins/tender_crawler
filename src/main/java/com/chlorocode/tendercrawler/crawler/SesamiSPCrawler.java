...
/**
 * Crawler specific implementation for Sesami SP.
 */
public class SesamiSPCrawler extends Crawler {

    final static Logger logger = LoggerFactory.getLogger(SesamiSPCrawler.class);
    final String URL = "https://sg.sesami.net/sp/businessOpportunities.jsp";
    final int SOURCE_ID = 4;

    public void run() {
        logger.info("Starting Sesami SP Crawler");
                ...
                for (HtmlTableRow row : table.getRows()) {
                    try {
                        ...
                        if (row.getCells().size() == 7) {
                            ...
                            logger.error("Table column size not expected. Expected: 7, Actual: " + row.getCells().size());
                        }
                    } catch (Exception ex) {
