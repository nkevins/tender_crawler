...
public class SesamiNHGCrawler extends Crawler {
    ...
    final String URL = "https://sg.sesami.net/nhg/businessOpportunities.jsp";
    final int SOURCE_ID = 2;

    public void run() {
        logger.info("Starting Sesami NHG Crawler");
                ...
                for (HtmlTableRow row : table.getRows()) {
                    try {
                        ...
                        if (row.getCells().size() == 9) {
                            ...
                            logger.error("Table column size not expected. Expected: 9, Actual: " + row.getCells().size());
                        }
                    } catch (Exception ex) {
